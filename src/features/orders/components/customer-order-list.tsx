...
export function CustomerOrderList() {
  ...
  if (orders.length === 0) {
    return (
      <section className="flex min-h-80 flex-col items-center justify-center rounded-2xl border border-dashed border-neutral-300 text-center dark:border-neutral-700">
        <PackageSearch aria-hidden={true} size={42} className="text-neutral-400" />

        <h1 className="mt-4 text-2xl font-semibold">No orders yet</h1>
  ...
  return (
    <section>
      <p className="text-sm font-medium text-neutral-500">Purchase history</p>

      <h1 className="mt-1 text-3xl font-semibold">My orders</h1>
          ...
          >
            <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
              ...
              <div className="flex flex-wrap items-center gap-4">
                ...
                <Link href={`/orders/${order._id}`} className="text-sm font-semibold underline">
                  View details
                </Link>
              </div>
            </div>
